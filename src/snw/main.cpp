...
// identifies a bit within a bit tree
...
private:
    ...
    template<cursor_mode mode>
    class cursor {
    public:
        cursor(bit_tree& bt)
            : bt_(bt)
        {
            if (mode == cursor_mode::set) {
                assert(!bt_.empty());
            }
            else {
                assert(!bt_.full());
...

            {
                ...
                for (size_t current_depth = 0; current_depth < (height - 1); ++current_depth) {
                    ...
                    if (mode == cursor_mode::set) {
                        chunks = branch.any_set_chunks;
                    }
                    else {
                        chunks = branch.any_clear_chunks;
                    }
            ...
            // traverse the leaf level
            {
                ...
                // scan chunks
                for (size_t chunk_index = 0; chunk_index < bit_tree_node::chunk_count; ++chunk_index) {
                    uint64_t chunk = leaf.chunks[chunk_index];
                    if (mode == cursor_mode::clear) {
        ...
        cursor(bit_tree& bt, uint32_t index)
            : bt_(bt)
            , index_(index)
        {
            if (mode == cursor_mode::set) {
